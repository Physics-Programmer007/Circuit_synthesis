...
parser.add_argument('--predictor', action='store_true', default=False,
                    help='whether to train a performance predictor from latent encodings and a VAE at the same time')
        ...
        if len(g_batch) == args.batch_size or i == len(train_data) - 1:
            ...
            if args.all_gpus:  # does not support predictor yet
                ...
                pbar.set_description(
                    'Epoch: %d, loss: %0.4f, recon: %0.4f, kld: %0.4f, type loss: %0.4f, pos loss: %0.4f,all df_loss: %0.4f' % (
...
r_valid_dag, r_valid_ckt, r_novel = prior_validity(train_data, model, infer_batch_size=args.infer_batch_size,
                                                   data_type=data_type, subg=subg, device=device,
                                                   scale_to_train_range=True)

test_results_name = os.path.join(args.res_dir, 'decode_results.txt')
with open(test_results_name, 'a') as result_file:
    result_file.write(
        " recon acc: {:.4f} r_valid_dag: {:.4f} r_valid_ckt: {:.4f} r_novel: {:.4f}\n".format(acc, r_valid_dag,
                                                                                              r_valid_ckt,
                                                                                              r_novel))
