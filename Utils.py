...
def is_valid_Circuit(g, subg=True):
    ...
    if subg:
        ...
        diameter_path = g.get_diameter(
            directed=True)  # find the main path the diameter path must start/end at the sudo input/end node
...
def prior_validity(train_data, model, infer_batch_size=64, data_type='igraph', subg=True, device=None,
                   scale_to_train_range=False):
        ...
        for g_id in tqdm(range(n_graph)):
            ...
            for i in range(num_subg):
                ...
                #    subg_flat_adj = [1]
                    ...
                    subg_flat_adj = [int(row[w]) for w in range(5 + num_edges + 2 * subg_nod,
                                                                5 + num_edges + 2 * subg_nod + subg_nod * subg_nod)]
                    # print(subg_flat_adj)
